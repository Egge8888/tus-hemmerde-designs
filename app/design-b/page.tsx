...
export default function DesignB() {
  return (
    <div style={{ backgroundColor: "#faf7f2", color: "#1c1917", fontFamily: "'Segoe UI', system-ui, sans-serif", minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", height: 72, display: "flex", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 28, fontSize: 15, fontWeight: 600 }}>
            {["Aktuelles", "Sport", "Mitglied werden", "Kontakt"].map((item) => (
              <a key={item} href="#" style={{ color: "#57534e", textDecoration: "none", padding: "4px 0", borderBottom: "2px solid transparent", transition: "all 0.2s" }}
          ...
          <a href="#" style={{ marginLeft: 24, backgroundColor: "#dc2626", color: "#fff", fontWeight: 700, fontSize: 14, padding: "10px 22px", borderRadius: 20, textDecoration: "none", boxShadow: "0 4px 12px rgba(220,38,38,0.3)" }}>
            Mitglied werden
          </a>
        </div>
      </nav>
      ...
      {/* HERO */}
      <section style={{ position: "relative", overflow: "hidden", background: "linear-gradient(135deg, #faf7f2 0%, #fff1f2 40%, #ffe4e6 70%, #fecdd3 100%)", minHeight: "88vh", display: "flex", alignItems: "center" }}>
        <div style={{ position: "absolute", top: -80, right: -80, width: 500, height: 500, borderRadius: "50%", backgroundColor: "rgba(220,38,38,0.08)", pointerEvents: "none" }}></div>
        <div style={{ position: "absolute", bottom: -120, left: "30%", width: 400, height: 400, borderRadius: "50%", backgroundColor: "rgba(185,28,28,0.06)", pointerEvents: "none" }}></div>
        <div style={{ position: "absolute", top: "20%", right: "15%", width: 60, height: 60, borderRadius: "50%", backgroundColor: "#dc2626", opacity: 0.2 }}></div>
        <div style={{ position: "absolute", top: "60%", right: "25%", width: 30, height: 30, borderRadius: "50%", backgroundColor: "#991b1b", opacity: 0.25 }}></div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "grid", gridTemplateColumns: "repeat(3, 1fr)" }}>
          {[
            ...
            <div key={stat.label} style={{ padding: "36px 24px", textAlign: "center", borderRight: i < 2 ? "2px solid #fee2e2" : "none" }}>
              ...
              <div style={{ fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 800, color: "#dc2626", lineHeight: 1, letterSpacing: "-0.03em" }}>{stat.value}</div>
      ...
      <section style={{ padding: "96px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, backgroundColor: "#fee2e2", color: "#991b1b", fontSize: 13, fontWeight: 600, padding: "6px 16px", borderRadius: 99, marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20 }}>
            {sports.map((sport) => (
              <div key={sport.name}
                ...
                <div style={{ width: 64, height: 64, borderRadius: 16, backgroundColor: sport.color + "15", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, marginBottom: 20 }}>
                  {sport.emoji}
                </div>
                <h3 style={{ fontSize: 20, fontWeight: 700, color: "#1c1917", marginBottom: 8 }}>{sport.name}</h3>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 99, backgroundColor: sport.color + "18", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: sport.color }}>
                    {sport.members.replace("+", "")}
                  </div>
                  <span style={{ fontSize: 14, color: "#78716c" }}>{sport.members} Mitglieder</span>
                </div>
                <div style={{ marginTop: 20, height: 3, borderRadius: 99, backgroundColor: sport.color + "20", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: "72%", borderRadius: 99, backgroundColor: sport.color }}></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: "96px 24px", backgroundColor: "#fff" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 48, flexWrap: "wrap", gap: 16 }}>
            ...
            <a href="#" style={{ color: "#dc2626", fontWeight: 700, textDecoration: "none", fontSize: 15 }}>Alle Nachrichten →</a>
          </div>

          {/* Magazine layout: 1 large + 2 small */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "auto auto", gap: 20 }}>
            <div style={{ gridRow: "1 / 3", backgroundColor: "#faf7f2", borderRadius: 24, overflow: "hidden", border: "2px solid #fee2e2", cursor: "pointer" }} className="news-card-b">
              <div style={{ height: 240, background: "linear-gradient(135deg, #dc2626 0%, #991b1b 60%, #7f1d1d 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 80 }}>
                📋
              </div>
              <div style={{ padding: 32 }}>
                <div style={{ display: "inline-block", backgroundColor: news[0].tagColor + "18", color: news[0].tagColor, fontSize: 12, fontWeight: 700, padding: "4px 12px", borderRadius: 99, marginBottom: 16 }}>
                  {news[0].tag}
                </div>
                ...
                <a href="#" style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 24, color: "#dc2626", fontWeight: 700, textDecoration: "none", fontSize: 14 }}>Mehr lesen →</a>
              </div>
            </div>
            {news.slice(1).map((item, i) => (
              <div key={i} style={{ backgroundColor: "#faf7f2", borderRadius: 20, padding: 28, border: "2px solid #fee2e2", cursor: "pointer", display: "flex", gap: 20, alignItems: "flex-start" }} className="news-card-b">
                <div style={{ width: 52, height: 52, borderRadius: 14, backgroundColor: item.tagColor + "15", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>
                  {i === 0 ? "📅" : "👋"}
                </div>
                <div>
                  <div style={{ display: "inline-block", backgroundColor: item.tagColor + "18", color: item.tagColor, fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 99, marginBottom: 8 }}>
                    {item.tag}
                  </div>
      ...
      <section style={{ padding: "96px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ background: "linear-gradient(135deg, #dc2626 0%, #b91c1c 40%, #991b1b 100%)", borderRadius: 28, padding: "80px 48px", textAlign: "center", position: "relative", overflow: "hidden", boxShadow: "0 24px 60px rgba(220,38,38,0.3)" }}>
            <div style={{ position: "absolute", top: -60, right: -60, width: 280, height: 280, borderRadius: "50%", backgroundColor: "rgba(255,255,255,0.08)" }}></div>
            <div style={{ position: "absolute", bottom: -80, left: -40, width: 320, height: 320, borderRadius: "50%", backgroundColor: "rgba(255,255,255,0.05)" }}></div>
            <div style={{ position: "relative", zIndex: 10 }}>
              ...
              <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
                ...
                <a href="#" style={{ backgroundColor: "rgba(255,255,255,0.15)", color: "#fff", fontWeight: 700, fontSize: 16, padding: "16px 32px", borderRadius: 99, textDecoration: "none", border: "2px solid rgba(255,255,255,0.35)" }}>
                  Alle Sportarten →
                </a>
              </div>
            </div>
      ...
      {/* FOOTER */}
      <footer style={{ backgroundColor: "#fff5f5", borderTop: "2px solid #fecdd3", padding: "64px 24px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      <style>{`
        .sport-card-b:hover { border-color: #dc2626 !important; box-shadow: 0 12px 32px rgba(220,38,38,0.15) !important; transform: translateY(-3px); }
